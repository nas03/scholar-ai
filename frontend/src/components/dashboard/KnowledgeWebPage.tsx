...
import { Network, ZoomIn, ZoomOut, Maximize2, BookOpen, Link2, Play, Pause, Minimize2 } from "lucide-react";
...
export function KnowledgeWebPage() {
  ...
  // Mocked notes data
  const notes: Note[] = [
    { id: "1", title: "Introduction to Algorithms", courseCode: "CS101", tags: ["Algorithms", "Theory", "Basics"] },
    { id: "2", title: "Sorting Algorithms", courseCode: "CS101", tags: ["Algorithms", "Practice", "Sorting"] },
    { id: "3", title: "Binary Trees", courseCode: "CS202", tags: ["Data Structures", "Trees", "Algorithms"] },
    { id: "4", title: "Graph Theory", courseCode: "CS202", tags: ["Data Structures", "Graphs", "Theory"] },
    { id: "5", title: "Calculus Basics", courseCode: "MATH201", tags: ["Mathematics", "Calculus", "Theory"] },
    { id: "6", title: "Derivatives", courseCode: "MATH201", tags: ["Mathematics", "Calculus", "Practice"] },
    { id: "7", title: "Newton's Laws", courseCode: "PHY102", tags: ["Physics", "Mechanics", "Theory"] },
    { id: "8", title: "Dynamic Programming", courseCode: "CS101", tags: ["Algorithms", "Optimization", "Practice"] },
    { id: "9", title: "Hash Tables", courseCode: "CS202", tags: ["Data Structures", "Hashing", "Practice"] },
    { id: "10", title: "Big O Notation", courseCode: "CS101", tags: ["Algorithms", "Theory", "Complexity"] },
  ];
  ...
  useEffect(() => {
    ...
    node.append("circle")
      .attr("r", d => {
        const connectionCount = d3Links.filter(l => 
    ...
    label.append("rect")
      .attr("x", d => {
        const text = d.title.length > 20 ? d.title.slice(0, 20) + '...' : d.title;
    ...
    node.on("mouseenter", function(event, d) {
      ...
      link.attr("stroke", l => {
        ...
        return (sourceId === d.id || targetId === d.id) ? "#3b82f6" : "#9ca3af";
      })
      ...
      .attr("stroke-width", l => {
        const sourceId = typeof l.source === 'object' ? l.source.id : l.source;
        const targetId = typeof l.target === 'object' ? l.target.id : l.target;
        return (sourceId === d.id || targetId === d.id) ? 2 + l.strength : 1 + l.strength * 0.5;
      });
    })
  ...
  }, [filteredNotes, filteredConnections, isSimulating, repulsionStrength, attractionStrength, isFullscreen]);

  // Get connections for selected note
  const selectedNoteConnections = selectedNote
    ? connections.filter(c => c.source === selectedNote.id || c.target === selectedNote.id)
    : [];
  ...
  return (
    <div className={`${isFullscreen ? 'fixed inset-0 z-50 bg-background' : 'p-6 max-w-7xl mx-auto'}`}>
      {/* Header */}
      <div className={`flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6 ${isFullscreen ? 'p-6 pb-0' : ''}`}>
        <div>
          <h2 className="text-2xl mb-1">Knowledge Web</h2>
          <p className="text-muted-foreground">
            Interactive graph view of note connections based on shared tags (powered by d3.js)
          </p>
        </div>
      ...
      {/* Stats */}
      <div className={`grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 ${isFullscreen ? 'px-6' : ''}`}>
        <Card className="p-4">
      ...
      <div className={`grid grid-cols-1 ${isFullscreen ? 'lg:grid-cols-4' : 'lg:grid-cols-3'} gap-6 ${isFullscreen ? 'px-6 pb-6 h-[calc(100vh-280px)]' : ''}`}>
        {/* SVG Canvas */}
        <div className={`${isFullscreen ? 'lg:col-span-3' : 'lg:col-span-2'} ${isFullscreen ? 'h-full' : ''}`}>
          <Card className={`p-6 ${isFullscreen ? 'h-full flex flex-col' : ''}`}>
            ...
            <div ref={containerRef} className={`relative bg-white border border-border rounded-lg overflow-hidden shadow-sm ${isFullscreen ? 'flex-1' : ''}`}>
              <svg
                ref={svgRef}
              ...
              <div className="absolute top-4 left-4 bg-card/95 backdrop-blur-sm p-3 rounded-lg border border-border">
                <p className="text-xs text-muted-foreground">
                  • Drag nodes to reposition • Scroll/pinch to zoom • Click & drag to pan
                </p>
              </div>
            </div>

            <p className="text-sm text-muted-foreground mt-4 text-center">
              Node size represents connection count • Hover to see details • Click to select
            </p>
          </Card>
        </div>
        ...
        <div className={`space-y-4 ${isFullscreen ? 'overflow-y-auto' : ''}`}>
          {selectedNote ? (
            <>
              ...
              <Card className="p-4">
                <h4 className="mb-3">Connected Notes ({selectedNoteConnections.length})</h4>
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {selectedNoteConnections.map(conn => {
                    const connectedId = conn.source === selectedNote.id ? conn.target : conn.source;
                    ...
                    return connectedNote ? (
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          <p className="text-sm flex-1">{connectedNote.title}</p>
                        ...
                        <div className="flex items-center gap-1">
                          <p className="text-xs text-muted-foreground">
                            {conn.sharedTags.length} shared tag{conn.sharedTags.length > 1 ? 's' : ''}:
                          </p>
            ...
            <Card className="p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {filteredNotes.length} notes • {filteredConnections.length} connections
              </p>
            </Card>
          )}
